...
import { Mail, Eye, MousePointer, AlertTriangle, Loader2 } from "lucide-react"
...
export default function EmailAnalytics() {
  ...
  const fetchEmailAnalytics = async () => {
    try {
      ...
      const stats: EmailStats = {
        ...
        delivered: emailLogs?.filter((log) => log.status === "delivered").length || 0,
        opened: emailLogs?.filter((log) => log.status === "opened").length || 0,
        clicked: emailLogs?.filter((log) => log.status === "clicked").length || 0,
        bounced: emailLogs?.filter((log) => log.status === "bounced").length || 0,
        spam: emailLogs?.filter((log) => log.status === "spam").length || 0,
      }
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="bg-white shadow-sm border-0">
        <CardHeader>
          <CardTitle className="text-lg font-bold text-[#212121]">Recent Email Activity</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-3">
            {recentEmails.map((email) => (
              <div key={email.id} className="flex items-center justify-between p-3 bg-[#F9F7F1] rounded-lg">
                <div className="flex-1">
                  <p className="text-sm font-medium text-[#212121]">{email.subject}</p>
                  <p className="text-xs text-[#212121] opacity-70">
                    To: {email.recipient_name || email.recipient_email} • {new Date(email.sent_at).toLocaleDateString()}
                  </p>
                </div>
                <Badge className={`${getStatusColor(email.status)} border flex items-center space-x-1`}>
            ...
            {recentEmails.length === 0 && (
              <p className="text-center text-[#212121] opacity-70 py-8">No emails sent yet</p>
            )}
          </div>
        </CardContent>
      </Card>
